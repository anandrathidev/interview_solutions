...
void Print_and_incr_t1()
{
    ...
    while(i<100)
    {
        try{
            ...
            std::cout << "Acq lock   and wait condi producer 1  i :" << i << "\n";
            c1.wait(lg);
            ...
            lg.unlock();
            c2.notify_one();
            std::cout << "Unlock and  notify producer 2  :" << "\n";
        }
...
void Print_and_incr_t2()
{
    while(i<100)
    {
        try{
            ...
            std::unique_lock<std::mutex> lg{mi};
            std::cout << "Acq lock   and wait condi producer 2  i :" << i << "\n";
            ...
            lg.unlock();
            c3.notify_one();
            std::cout << "Unlock and notify producer 3  "  << "\n";
        }
        catch(std::exception &e)
        {
            std::cout << e.what() << "\n";
...
void Print_and_incr_t3()
{
    while(i<100)
    {
        try{
            std::unique_lock<std::mutex> lg{mi};
            std::cout << "Acq lock  and wait condi  producer 3 :" << i << "\n";
            c3.wait(lg);
            ++i;
            std
            ::cout << "producer 3  i :" << i << "\n";
            lg.unlock();
            std::cout << "Unlock and notify producer 1  :" << "\n";
            c1.notify_one();
        }
        catch(std::exception &e)
        {
